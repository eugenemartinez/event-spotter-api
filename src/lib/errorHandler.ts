...
function transformFastifyZodValidationToFieldErrors(validationErrors: any[]): Record<string, string[]> {
...
export function registerErrorHandler(server: FastifyInstance): void {
  server.setErrorHandler((error: FastifyError, request: FastifyRequest, reply: FastifyReply) => {
    ...
    if (error instanceof ZodError) {
        request.log.warn({ err: error.flatten(), details: error.issues }, 'Direct ZodError instance caught by errorHandler.ts');
    ...
    if (error instanceof Prisma.PrismaClientKnownRequestError) {
      request.log.warn({ err: error }, `PrismaClientKnownRequestError in errorHandler.ts: ${error.code}`);
      if (error.code === 'P2002') {
      ...
      return reply.status(500).send({ message: 'A database error occurred processing your request.' });
    }
    ...
    const responseMessage = isProduction ? "An unexpected error occurred on the server." : error.message;
    const errorName = isProduction ? "Internal Server Error" : error.name || "Internal Server Error";
    // Ensure statusCode is a valid HTTP status code, default to 500
    const finalStatusCode = (typeof statusCode === 'number' && statusCode >= 100 && statusCode <= 599) ? statusCode : 500;
