...
import {
  createEventSchema,
  CreateEventInput, // Import Zod-inferred type
  apiEventResponseSchema,
  eventParamsSchema,
  EventParams, // Import Zod-inferred type
  updateEventSchema,
  UpdateEventInput, // Import Zod-inferred type
  eventListQuerySchema,
  EventListQuery, // Import Zod-inferred type
  paginatedEventsResponseSchema,
  successMessageResponseSchema,
  eventCategoriesResponseSchema,
  eventTagsResponseSchema,
  batchGetEventsBodySchema,
  BatchGetEventsBody, // Import Zod-inferred type
...
interface AuthenticatedUser {
  // This should ideally be the AuthenticatedUser from src/types/index.ts
  id: string;
  username: string;
}

const eventRoutes: FastifyPluginAsync<
      ...
      config: {
        rateLimit: {
          ...
          keyGenerator: function (request: FastifyRequest) {
            // FastifyRequest here is generic
            const user = request.user as AuthenticatedUser | undefined;
            if (user && user.id) {
              return user.id;
            }
            server.log.warn(
              { path: request.raw.url, ip: request.ip, userPayload: request.user },
              'Rate limit key generator for POST /api/events falling back to IP. User object was not as expected or missing id.',
            );
            return request.ip;
          },
        },
  ...
  server.get<{ Querystring: EventListQuery }>(
    '/',
    {
      schema: {
        ...
        description:
          'Retrieves a paginated list of events. Supports filtering, sorting, and searching.',
        tags: ['Events'],
  ...
  server.patch<{ Params: EventParams; Body: UpdateEventInput }>( // Explicitly type here
    '/:eventId',
    {
      ...
      schema: {
        ...
        description:
          'Updates specified fields of an existing event. Requires authentication and ownership.',
        tags: ['Events'],
        params: eventParamsSchema,
      ...
      config: {
        rateLimit: {
          ...
          keyGenerator: function (request: FastifyRequest) {
            ...
            server.log.warn(
              { path: request.raw.url, ip: request.ip, userPayload: request.user },
              'Rate limit key generator for PATCH /api/events/:eventId falling back to IP. User object was not as expected or missing id.',
            );
            return request.ip;
          },
        },
      ...
      config: {
        rateLimit: {
          ...
          keyGenerator: function (request: FastifyRequest) {
            ...
            server.log.warn(
              { path: request.raw.url, ip: request.ip, userPayload: request.user },
              'Rate limit key generator for DELETE /api/events/:eventId falling back to IP. User object was not as expected or missing id.',
            );
            return request.ip;
          },
        },
  ...
  server.delete<{ Params: EventParams }>( // Explicitly type here
    '/:eventId/save',
    {
      ...
      schema: {
        ...
        description:
          'Allows an authenticated user to remove an event from their list of saved events.',
        tags: ['Events', 'User Actions'],
        params: eventParamsSchema,
        response: {
          204: z
            .null()
            .describe('Event unsaved successfully or was not saved. No content returned.'),
          400: errorResponseSchema400,
          401: errorResponseSchema401,
          404: errorResponseSchema404,
          500: errorResponseSchema500,
  ...
  server.post<{ Body: BatchGetEventsBody }>(
    '/batch-get',
    {
      schema: {
        ...
        response: {
          200: batchGetEventsResponseSchema,
          400: errorResponseSchema400,
          500: errorResponseSchema500,
        },
      },
