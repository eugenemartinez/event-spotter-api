...
// Mock the prisma client using importOriginal
    ...
    default: {
      ...
      $transaction: vi.fn((args) => {
        if (Array.isArray(args)) {
          // Simulate transaction behavior for array of promises
          return Promise.all(
            args.map((op) => {
              if (typeof op.then === 'function') return op; // Already a promise
              // This is a simplified mock, adjust if complex operations are needed
        ...
        // For callback style transaction
        if (typeof args === 'function') {
          return args(actualModule.default); // Pass the original mock client to the callback
        }
...
vi.mock('./event.controllers', async (importOriginal) => {
  ...
  return {
    ...
    transformEventForApi: vi.fn(), // Mock specific function
  };
});
...
const mockRequest = <T extends RouteGenericInterface = RouteGenericInterface>(
  ...
  log: {
    ...
    trace: vi.fn(),
    fatal: vi.fn(),
    child: vi.fn(() => mockRequest(data).log as any),
    level: 'info',
    silent: vi.fn(),
  },
  ...data,
...
> => {
  ...
  reply.log = {
    ...
    trace: vi.fn(),
    fatal: vi.fn(),
    child: vi.fn(() => mockReply<T>().log as any),
    level: 'info',
    silent: vi.fn(),
  };
  return reply;
...
describe('Event Controllers Unit Tests', () => {
  let prisma: any; // To access the mocked prisma client instance

  beforeEach(async () => {
    // Make beforeEach async if needed for dynamic import
    vi.clearAllMocks();
    // Dynamically import the mocked prisma client for use in tests
    // This ensures we get the version affected by vi.mock
  ...
  describe('saveEventHandler', () => {
    const mockAuthenticatedUser = { id: 'user-save-event-id', username: 'eventSaver' };
    const mockEventIdToSave = 'event-to-be-saved-uuid';
    const mockNonExistentEventId = 'non-existent-event-to-save-uuid';

    beforeEach(() => {
      // Reset mocks for Prisma operations relevant to saveEventHandler
      (prisma.event.count as import('vitest').Mock).mockReset();
      (prisma.userSavedEvent.findUnique as import('vitest').Mock).mockReset();
      (prisma.userSavedEvent.create as import('vitest').Mock).mockReset();
      // Reset any logger spies if they are set up per test or in a broader beforeEach
    });
    ...
    it('should save an event for an authenticated user successfully', async () => {
      ...
      // Spy on req.log.info (if any info logs were expected, though saveEventHandler primarily logs errors)
      if (!req.log) {
        req.log = {
          error: vi.fn(),
          warn: vi.fn(),
        ...
        // Add spies for other log levels if needed
      }

      // 1. Mock event.count to indicate the event exists
      (prisma.event.count as import('vitest').Mock).mockResolvedValue(1);

      // 2. Mock userSavedEvent.findUnique to indicate the user has NOT already saved this event
      (prisma.userSavedEvent.findUnique as import('vitest').Mock).mockResolvedValue(null);

      // 3. Mock userSavedEvent.create to simulate successful creation
      //    The actual created object isn't used by the handler, so an empty object or mock is fine.
      (prisma.userSavedEvent.create as import('vitest').Mock).mockResolvedValue({
        userId: mockAuthenticatedUser.id,
        eventId: mockEventIdToSave,
        savedAt: new Date(),
      });
      ...
      // Assertions:
      // Check that prisma.event.count was called correctly
      expect(prisma.event.count).toHaveBeenCalledWith({
        where: { id: mockEventIdToSave },
      });

      // Check that prisma.userSavedEvent.findUnique was called correctly
      expect(prisma.userSavedEvent.findUnique).toHaveBeenCalledWith({
        where: { userId_eventId: { userId: mockAuthenticatedUser.id, eventId: mockEventIdToSave } },
      });

      // Check that prisma.userSavedEvent.create was called correctly
      expect(prisma.userSavedEvent.create).toHaveBeenCalledWith({
        data: { userId: mockAuthenticatedUser.id, eventId: mockEventIdToSave },
      });
    ...
    it('should return 404 if event to save is not found (event.count is 0)', async () => {
      ...
      // Spy on req.log.error to ensure it's not called for this specific 404 path
      if (!req.log) {
        req.log = {
          error: vi.fn(),
          warn: vi.fn(),
      ...
      // 1. Mock event.count to indicate the event does NOT exist
      (prisma.event.count as import('vitest').Mock).mockResolvedValue(0);
      ...
      // Check that subsequent Prisma operations were NOT called
      expect(prisma.userSavedEvent.findUnique).not.toHaveBeenCalled();
      expect(prisma.userSavedEvent.create).not.toHaveBeenCalled();

      // Check for the 404 Not Found response
      expect(rep.code).toHaveBeenCalledWith(404);
      expect(rep.send).toHaveBeenCalledWith({ message: 'Event not found.' });

      // Ensure no error was logged for this specific 404 path
      // The controller returns directly without hitting the main catch block's log.error
      expect(req.log.error).not.toHaveBeenCalled();
    });
    ...
    it('should return 200 if event is already saved by the user', async () => {
      ...
      // 2. Mock userSavedEvent.findUnique to indicate the user HAS already saved this event
      const mockExistingSave = {
        userId: mockAuthenticatedUser.id,
        eventId: mockEventIdToSave,
        savedAt: new Date(),
      };
      (prisma.userSavedEvent.findUnique as import('vitest').Mock).mockResolvedValue(
        mockExistingSave,
      );
      ...
      // Check that prisma.userSavedEvent.create was NOT called
      expect(prisma.userSavedEvent.create).not.toHaveBeenCalled();
    ...
    it('should return 500 if Prisma event.count operation itself fails', async () => {
      ...
      const dbOperationError = new Error('Database connection lost during event.count');
      // 1. Mock event.count to reject with an error
      (prisma.event.count as import('vitest').Mock).mockRejectedValue(dbOperationError);

      // Call the handler
      await saveEventHandler(req, rep);

      // Assertions:
      // Check that prisma.event.count was called
      expect(prisma.event.count).toHaveBeenCalledWith({
        where: { id: mockEventIdToSave },
      });
      ...
      expect(req.log.error).toHaveBeenCalledWith(
        { error: dbOperationError, eventId: mockEventIdToSave, userId: mockAuthenticatedUser.id },
        'Error saving event',
      );
    });
    ...
    it('should return 500 if Prisma fails to create UserSavedEvent relation (other than P2003 or P2002 handled by prior checks)', async () => {
      ...
      // 3. Mock userSavedEvent.create to reject with a generic error
      const createRelationError = new Error(
        'Failed to create UserSavedEvent due to an unexpected DB issue.',
      );
      (prisma.userSavedEvent.create as import('vitest').Mock).mockRejectedValue(
        createRelationError,
      );
      ...
      // Check that prisma.userSavedEvent.findUnique was called
      expect(prisma.userSavedEvent.findUnique).toHaveBeenCalledWith({
        where: { userId_eventId: { userId: mockAuthenticatedUser.id, eventId: mockEventIdToSave } },
      });

      // Check that prisma.userSavedEvent.create was called
      expect(prisma.userSavedEvent.create).toHaveBeenCalledWith({
        data: { userId: mockAuthenticatedUser.id, eventId: mockEventIdToSave },
      });
    ...
    it('should return 404 if creating UserSavedEvent fails with P2003 (foreign key constraint)', async () => {
      ...
      // 1. Mock event.count to indicate the event "exists" (passes the first check)
      (prisma.event.count as import('vitest').Mock).mockResolvedValue(1);

      // 2. Mock userSavedEvent.findUnique to indicate the user has NOT already saved this event
      (prisma.userSavedEvent.findUnique as import('vitest').Mock).mockResolvedValue(null);

      // 3. Mock userSavedEvent.create to reject with a P2003 error
      const p2003Error = new Prisma.PrismaClientKnownRequestError(
        'Foreign key constraint failed on the field: `eventId`', // Example message
        { code: 'P2003', clientVersion: 'test-version', meta: { field_name: 'eventId' } }, // Meta might vary
      );
      (prisma.userSavedEvent.create as import('vitest').Mock).mockRejectedValue(p2003Error);
      ...
      // Check for the 404 Not Found response due to P2003
      expect(rep.code).toHaveBeenCalledWith(404);
      expect(rep.send).toHaveBeenCalledWith({ message: 'Event not found or user invalid.' });

      // Check that the error was logged
      expect(req.log.error).toHaveBeenCalledWith(
        { error: p2003Error, eventId: mockEventIdToSave, userId: mockAuthenticatedUser.id },
        'Error saving event',
      );
    });
    ...
    it('should return 500 for unexpected errors', async () => {
      ...
      // 3. Mock userSavedEvent.create to reject with a very generic, unexpected error
      const unexpectedError = new Error('A completely unexpected runtime error occurred!');
      (prisma.userSavedEvent.create as import('vitest').Mock).mockRejectedValue(unexpectedError);
      ...
      // Check that the error was logged
      expect(req.log.error).toHaveBeenCalledWith(
        { error: unexpectedError, eventId: mockEventIdToSave, userId: mockAuthenticatedUser.id },
        'Error saving event',
      );
    });
  });
